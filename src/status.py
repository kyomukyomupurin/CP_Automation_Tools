...
def get_status() -> None:
    ...
    for tr in tbody.find_all("tr"):
        ...
        flag_CE: bool = len(submission_info) == 6
        if flag_CE:
            (submission_time, task, language, score, code_size,
             status) = submission_info
            print(
                f" {submission_time[:-5]}   {task[0]}   {language:>15}   {score:>4}   {code_size:>11}   {Color.YELLOW}{status:>3}{Color.DEFAULT}")
        else:
            (submission_time, task, language, score, code_size,
